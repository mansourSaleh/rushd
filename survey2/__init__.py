...
class Intro(Page):
    ...
    def before_next_page(player: Player, timeout_happened):
        if player.id_in_group % 3 == 0:
           player.user_group = 4 # controled => bank defaul => 15,000
        elif player.id_in_group % 3 == 1:
            player.user_group = 4 # three choices not images;
        else:
            player.user_group = 4 # with images
...
class PaymentAmount(Page):
    form_model = 'player'
    form_fields = ['monthly_installment_amount', 'register_in_zood', 'endorsement', 'bank_for_zood']
...
page_sequence = [Intro, FinancialAnalysis,  PersonalAnalysis ,Freelance,  Productive_Family,  FinancialRequest, PaymentAmount, Result, ]
